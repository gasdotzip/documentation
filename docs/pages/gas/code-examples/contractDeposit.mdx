# Code Example for Contract Deposit (Deprecated)

:::warning
**DEPRECATION NOTICE:** The v1 Deposit Contracts are being phased out. While still functional, we recommend using the newer [EOA Deposit method](/gas/code-examples/eoaDeposit) for future integrations.
:::

This section provides example implementations for calling the `deposit()` function on the v1 Deposit Contracts. These examples are maintained for legacy support but are not recommended for new integrations.
