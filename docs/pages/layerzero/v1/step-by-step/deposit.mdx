...
Use the `adapterParamsDeposit` and target address as arguments for the `deposit()` function. You must use the [aggregated value](/layerzero/step-by-step/estimateFees#call-the-estimatefees-function-and-aggregate-the-fees) `estimateFees()` generates as the `value` prop in the `deposit()` function.
